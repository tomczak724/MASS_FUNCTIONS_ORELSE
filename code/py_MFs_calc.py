...
voronoi_dir = '/Users/atomczak/Dropbox/ORELSE/DATA/VORONOI/old_prior_2_bugfix'
...
class field:
	def __init__(self, name, version, zclust, sigmaz, alpha=50, chi2red_thresh=10, uvj_slope=0.88, uvj_intercept=0.59):
		...
		self.voronoi = pickle.load(open('%s/%s_voronoi.pickle' % (voronoi_dir, name), 'rb'))
		self.overdens_max = []
...
for i in range(len(fields)):
	...
	###  writing output
	outer = open('../data/MF_%s_LSS.dat' % f.name, 'w')
	...
	for j in range(len(lmassbars)):
		...
		outer.write('\n')
	outer.close()
	print 'wrote to: ../data/MF_%s_LSS.dat' % f.name
	...
	if True:
		...
		###  writing output
		outer = open('../data/MF_%s_LSS_2filter.dat' % f.name, 'w')
		outer.write('#   Ntot = Nzspec + Nzphot_corr\n')
		outer.write('#   Nzspec = number of LSS members selected by zspec\n')
		outer.write('#   Nzphot = number of candidate LSS members selected in broad zphot bin\n')
		outer.write('#   Nzphot_corr = Nzphot corrected by false +/- fractions to estimate the\n')
		...
		for j in range(len(lmassbars)):
			...
			outer.write('\n')
		outer.close()
		print 'wrote to: ../data/MF_%s_LSS_2filter.dat' % f.name
...
for i in range(len(fields)):
	...
	for overi in range(1, len(overdens_bins)):
		...
		###  writing output
		outer = open('../data/MF_%s_LSS_%s.dat' % (f.name, overdens_labels[overi-1]), 'w')
		outer.write('#   Ntot = Nzspec + Nzphot_corr\n')
		...
		for mi in range(1, len(lmassbins)):
			...
			outer.write('\n')
		outer.close()
		print 'wrote to: ../data/MF_%s_LSS_%s.dat' % (f.name, overdens_labels[overi-1])
	print ''
...
outer_massiveGals = open('../output/massiveGals.txt' ,'w')
for fi in range(len(fields)):
	...
	pickle.dump(mf_field, open('../data/MF_%s_field.pickle' % f.name, 'wb'))
	print 'wrote to: ../data/MF_%s_field.pickle' % f.name
...
class massFunction_voronoi_slices:
	def __init__(self, name, zclust, sigmaz, area_zspec_arcmin2, zlos, zhis):
		...
		self.ngals_all_sf = [self.ngals_00v05_sf, self.ngals_05v10_sf, self.ngals_10v15_sf, self.ngals_15v20_sf]
		self.elo_ngals_all_sf = [self.elo_ngals_00v05_sf, self.elo_ngals_05v10_sf, self.elo_ngals_10v15_sf, self.elo_ngals_15v20_sf]
		self.ehi_ngals_all_sf = [self.ehi_ngals_00v05_sf, self.ehi_ngals_05v10_sf, self.ehi_ngals_10v15_sf, self.ehi_ngals_15v20_sf]
		self.phis_all_sf = [self.phis_00v05_sf, self.phis_05v10_sf, self.phis_10v15_sf, self.phis_15v20_sf]
		...
		self.ngals_all_qu = [self.ngals_00v05_qu, self.ngals_05v10_qu, self.ngals_10v15_qu, self.ngals_15v20_qu]
		self.elo_ngals_all_qu = [self.elo_ngals_00v05_qu, self.elo_ngals_05v10_qu, self.elo_ngals_10v15_qu, self.elo_ngals_15v20_qu]
		self.ehi_ngals_all_qu = [self.ehi_ngals_00v05_qu, self.ehi_ngals_05v10_qu, self.ehi_ngals_10v15_qu, self.ehi_ngals_15v20_qu]
		self.phis_all_qu = [self.phis_00v05_qu, self.phis_05v10_qu, self.phis_10v15_qu, self.phis_15v20_qu]
...
for i in range(len(fields)):
	...
	pickle.dump(mf_voronoi_slices_field, open('../data/MF_%s_voronoi_slices.pickle' % f.name, 'wb'))
	print '\nwrote to: ../data/MF_%s_voronoi_slices.pickle\n' % f.name
