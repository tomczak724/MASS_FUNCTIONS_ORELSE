...
if True:
    ...
    class mf_data:
        def __init__(self, name, zclust, sigmaz, voronoi_bin=''):
            ...
            self.mf_field = pickle.load(open('%s/MF_%s_field.pickle' % (data_dir, name), 'rb'))
            self.mf_voronoi_slices = pickle.load(open('%s/MF_%s_voronoi_slices.pickle' % (data_dir, name), 'rb'))
...
if False:
    ...
    pylab.savefig('../output_after_VoronoiMC_bugfix/massfunctions_EDisCS+GCLASS.png')
    pylab.close()
    print 'wrote to: ../output_after_VoronoiMC_bugfix/massfunctions_EDisCS+GCLASS.png'
...
if False:
    ...
    for i in range(len(plot_fields)):
        ...
        outname = '../output_after_VoronoiMC_bugfix/MF_%s.png' % f.name
        fig.savefig(outname)
...
if False:
    ...
    outname = '../output_after_VoronoiMC_bugfix/massFunctions_field.pdf'
    ...
    outname = '../output_after_VoronoiMC_bugfix/schechter_params.png'
    fig.savefig(outname)
    pylab.close()
    print 'wrote to: %s\n' % outname
...
if False:
    ...
    if True:
        ...
        asdf3 = sp2.errorbar(vdb_mstar, vdb_alpha, xerr=vdb_mstar_err, yerr=vdb_alpha_err, 
        ...
        asdf3 = sp2.errorbar(V_mstar, V_alpha, xerr=V_mstar_err, yerr=V_alpha_err, 
        ...
        asdf3 = sp2.errorbar(g_mstar, g_alpha, xerr=g_mstar_err, yerr=g_alpha_err, 
                            ls='', marker='s', ms=8, mec='k', mew=2, mfc='g', ecolor='k', label='COSMOS-Xray: 0.8<z<1.0')

        sp2.legend(loc=3, numpoints=1, fontsize=15)

        outname = '../output_after_VoronoiMC_bugfix/MF_voronoiBins.png'
        fig.savefig(outname)
        print 'wrote to: %s\n' % outname
        pylab.close()
        ...
        outname2 = '../output_after_VoronoiMC_bugfix/spec_completeness_vbins.png'
    ...
    outname = '../output_after_VoronoiMC_bugfix/MF_voronoiBins_sf.png'
    fig.savefig(outname)
    print 'wrote to: %s\n' % outname
    pylab.close()
    ...
    outname = '../output_after_VoronoiMC_bugfix/MF_voronoiBins_qu.png'
    fig.savefig(outname)
    print 'wrote to: %s\n' % outname
    pylab.close()
...
if False:
    ...
    outname = '../output_after_VoronoiMC_bugfix/massFunctions_from_summed_voronoi_slices.pdf'
    pdf = PdfPages(outname)
...
if True:
    ...
    #outname = '../output_after_VoronoiMC_bugfix/massFunctions_from_summed_voronoi_slices2.pdf'
    #pdf = PdfPages(outname)
        ...
        for zi in range(0, len(f.mf_voronoi_slices.zbars), 2):
            ...
            for mi in range(len(lmassbars)):
                ...
                if lmassbins[mi] > lmasslimit:
                    ...
                    #master_number_counts[fi][2][mi] += f.mf_voronoi_slices.ngals_field[zi][mi]
                    #master_volumes[fi][2][mi] += f.mf_voronoi_slices.volumes_field_Mpc3[zi]
                    ...
                    if f.mf_voronoi_slices.volumes_n10v05_Mpc3[zi] > 0: master_nslices[fi][0][mi] += 1
                    if f.mf_voronoi_slices.volumes_n05v00_Mpc3[zi] > 0: master_nslices[fi][1][mi] += 1
                    if f.mf_voronoi_slices.volumes_00v05_Mpc3[zi] > 0: master_nslices[fi][2][mi] += 1
                    if f.mf_voronoi_slices.volumes_05v10_Mpc3[zi] > 0: master_nslices[fi][3][mi] += 1
                    if f.mf_voronoi_slices.volumes_10v15_Mpc3[zi] > 0: master_nslices[fi][4][mi] += 1
                    if f.mf_voronoi_slices.volumes_15v20_Mpc3[zi] > 0: master_nslices[fi][5][mi] += 1

                if lmassbins[mi] > lmasslimit:
                    master_number_counts_sf[fi][0][mi] += f.mf_voronoi_slices.ngals_n05v00_sf[zi][mi]
                    master_volumes_sf[fi][0][mi] += f.mf_voronoi_slices.volumes_n05v00_Mpc3[zi]
                    ...
                    #master_number_counts_sf[fi][2][mi] += f.mf_voronoi_slices.ngals_field_sf[zi][mi]
                    #master_volumes_sf[fi][2][mi] += f.mf_voronoi_slices.volumes_field_Mpc3[zi]
                ...
                if lmassbins[mi] > lmasslimit_ssp:
                    master_number_counts_qu[fi][0][mi] += f.mf_voronoi_slices.ngals_n05v00_qu[zi][mi]
                    master_volumes_qu[fi][0][mi] += f.mf_voronoi_slices.volumes_n05v00_Mpc3[zi]
                    ...
                    #master_number_counts_qu[fi][2][mi] += f.mf_voronoi_slices.ngals_field_qu[zi][mi]
                    #master_volumes_qu[fi][2][mi] += f.mf_voronoi_slices.volumes_field_Mpc3[zi]
    ...
    for vi_counter, vi in enumerate(o_range):
        ...
        print 'runing on %s' % voronoi_labels[vi]



    #for vi in range(1, len(overdens_bars)):
    #for vi in [0]:
        ...
        try:
            ...
            ###  fitting a schechter function to SMF
            #fit, cov = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts.sum(axis=0) / master_volumes.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=(elo+ehi)[inds]/2.)
            ...
            fit, cov = optimize.curve_fit(dschechter, xdata, ydata, p0=[11, -1., -1., 10**-3, 10**-4], sigma=ydata_errs, maxfev=10**6)
            ...
            dy = pylab.log10(y[inds]) - pylab.log10(pylab.interp(x[inds], xmod, ymod) / dm)
            ey = (pylab.log10(y[inds] / (y[inds] - elo[inds])) + pylab.log10((y[inds] + ehi[inds]) / y[inds])) / 2.
            chi2reds.append(pylab.sum(dy**2 / ey**2) / (len(fit) - 1))
        ...
        fit_double, cov_double = optimize.curve_fit(dschechter, xdata, ydata, p0=[10.83, -0.5, -1.55, 10**-2.81, 10**-4.40], sigma=ydata_errs, maxfev=10**5)


        ###  fitting single
        fit_single, cov_single = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts.sum(axis=0) / master_volumes.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=((elo+ehi)[inds]/2.), maxfev=10**6)



        ###  calculating reduced chi**2  ---  single Schechter
        lymod = pylab.log10(schechter_mf(x[inds], *fit_single) / dm)
        lydata = pylab.log10(y[inds])
        lyerr = (elo+ehi)[inds]/2. / y[inds] / pylab.log(10)

        inds_chi2 = pylab.find(-pylab.isnan(lydata) & -pylab.isinf(lydata))

        chi2 = ((lymod[inds_chi2] - lydata[inds_chi2])**2 / lyerr[inds_chi2]**2).sum()
        chi2_single.append(chi2)



        ###  calculating reduced chi**2  ---  double Schechter
        lymod = pylab.log10(dschechter(x[inds], *fit_double) / dm)
        lydata = pylab.log10(y[inds])
        lyerr = (elo+ehi)[inds]/2. / y[inds] / pylab.log(10)

        inds_chi2 = pylab.find(-pylab.isnan(lydata) & -pylab.isinf(lydata))
        n_points.append(len(inds_chi2))

        chi2 = ((lymod[inds_chi2] - lydata[inds_chi2])**2 / lyerr[inds_chi2]**2).sum()
        chi2_double.append(chi2)
    ...
    chi2reds = pylab.array(chi2reds)

    chi2_single = numpy.array(chi2_single)
    chi2_double = numpy.array(chi2_double)
    ...
    if False:
        ...
        for vi in range(len(overdens_bars)):
            ...
            for f, c in zip(fits_tot[vi][order], covs_tot[vi][order]):
                s += '$%5.2f\pm%4.2f$ & ' % (f, c)
            print s[:-2] + ' \\\\'
    ...
    #for vi in [0]:
        ...
        sp1.errorbar(x[inds] + offie[vi], y[inds], xerr=dm/2., yerr=[elo[inds], ehi[inds]],
                     ls='', mew=1.5, marker='o', ms=9, mfc='none', mec=color_i, elinewidth=1.5, ecolor=color_i, label=voronoi_labels[vi])
        xmod = pylab.linspace(lmassbars[inds][0] - dm/2., lmassbars[inds][-1] + dm/2., 1000)
        ...
        ###  plotting ratio
        sp2.errorbar(x[inds] + offie[vi], y[inds] / ydatas[0][inds], xerr=dm/2., yerr=[elo[inds] / ydatas[0][inds], ehi[inds] / ydatas[0][inds]],
                         ls='', mew=1.5, marker='o', ms=9, mfc='none', mec=color_i, elinewidth=1.5, ecolor=color_i, label=voronoi_labels[vi])
        ...
        ###  fitting single
        fit_single, cov_single = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts_sf.sum(axis=0) / master_volumes_sf.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=((elo+ehi)[inds]/2.), maxfev=10**6)
    ...
    #for vi in [0]:
        ...
        elo = y * (ylo / n)
        ehi = y * (yhi / n)
        sp3.errorbar(x[inds] + offie[vi], y[inds], xerr=dm/2., yerr=[elo[inds], ehi[inds]],
                     ls='', mew=1.5, marker='o', ms=9, mfc='none', mec=color_i, elinewidth=1.5, ecolor=color_i, label=voronoi_labels[vi])
        xmod = pylab.linspace(lmassbars[inds][0] - dm/2., lmassbars[inds][-1] + dm/2., 1000)
        ...
        ###  plotting ratio
        sp4.errorbar(x[inds] + offie[vi], y[inds] / ydatas[0][inds], xerr=dm/2., yerr=[elo[inds] / ydatas[0][inds], ehi[inds] / ydatas[0][inds]],
                         ls='', mew=1.5, marker='o', ms=9, mfc='none', mec=color_i, elinewidth=1.5, ecolor=color_i, label=voronoi_labels[vi])
    ...
    sp3.legend(loc=3, numpoints=1, fontsize=16)
...
data_dir = '../data'
data_dir = '../data_after_VoronoiMC_bugfix'
class mf_data:
    def __init__(self, name, zclust, sigmaz, voronoi_bin=''):
        self.name = name
        self.zclust = zclust
...
plot_fields.append(mf_data('RXJ1221', 0.700,  0.023))
plot_fields.append(mf_data('CL1350',  0.804,  0.035))
plot_fields.append(mf_data('SG0023',  0.845,  0.025))
plot_fields.append(mf_data('CL1137',  0.959,  0.032))
plot_fields.append(mf_data('XLSS005', 1.000,  0.024))
plot_fields.append(mf_data('RXJ1053', 1.140,  0.031))
plot_fields.append(mf_data('CL1429',  0.920,  0.051))
...
if True:
        ...
        ngal_table_tot = pylab.zeros((len(lmassbars), 1 + 2*(len(overdens_bars)-1)))
        ngal_table_sf  = pylab.zeros((len(lmassbars), 1 + 2*(len(overdens_bars)-1)))
        ngal_table_qu  = pylab.zeros((len(lmassbars), 1 + 2*(len(overdens_bars)-1)))
        ...
        for fi in range(len(plot_fields)):
                ...
                for zi in range(0, len(f.mf_voronoi_slices.zbars), 2):
                        ...
                        ###  skipping redshifts of choice
                        if f.mf_voronoi_slices.zbars[zi] < 0.6: continue
                        if f.mf_voronoi_slices.zbars[zi] > 0.9: continue
        ...
        #for vi in [0]:
                ...
                inds = pylab.find((master_volumes.sum(axis=0)[vi] > 0) & \
                                  (master_number_counts.sum(axis=0)[vi] > 0) & \
                                  (lmassbars < mlim_hi))
                ...
                ngal_table_tot[:,0] = lmassbars
                ngal_table_tot[inds,2*vi_counter+1] = n[inds]
                ngal_table_tot[inds,2*vi_counter+2] = master_volumes.sum(axis=0)[vi][inds]
        ...
        n_points = pylab.array(n_points)
        chi2_single = pylab.array(chi2_single)
        chi2_double = pylab.array(chi2_double)
        ...
        outer = open('%s/massFunctions_TOT_Tomczak+2017_lowz.dat' % data_dir, 'w')
        ...
        ###  writing data_table to ascii file  ---  NUMBERS OF GALAXIES PER BIN
        outer = open('%s/numberCounts_TOT_Tomczak+2017_lowz.dat' % data_dir, 'w')

        outer.write('#    Galaxy stellar mass functions for all galaxies\n')
        outer.write('#\n')
        outer.write('#    Column  Units   Description\n')
        outer.write('#    lmass   Msol    Log stellar mass at center of mass-bin\n')
        outer.write('#    ngal    ----    Number of galaxies\n')
        outer.write('#    volume  cMpc^3  Comoving volume from which galaxies were counted\n')
        outer.write('#\n')
        outer.write('#    Overdensity bins\n')
        outer.write('#    _n05d00    -0.5 < log(1+delta) < 0.0\n')
        outer.write('#    _00d05      0.0 < log(1+delta) < 0.5\n')
        ...
        for vi_counter, vi in enumerate(o_range):
                if len(voronoi_bins[vi]) == 6:
                        outer.write('   ngal%s' % (voronoi_bins[vi]))
                        outer.write('   volume%s' % (voronoi_bins[vi]))
                if len(voronoi_bins[vi]) == 7:
                        outer.write('  ngal%s' % (voronoi_bins[vi]))
                        outer.write('  volume%s' % (voronoi_bins[vi]))
        outer.write('\n')

        for row in ngal_table_tot:
                outer.write('%6.2f ' % row[0])
                for item in row[1:]:
                        outer.write('  %8.3i' % item)
                outer.write('\n')
        ...
        #for vi in [0]:
                ...
                ###  fitting double
                p0 = [10.64, 0.12, -1.49, 5.7e-4, 2.6e-4]
                #fit_double, cov_double = optimize.curve_fit(dschechter, xdata, ydata, p0=[10.83, -0.5, -1.55, 10**-2.81, 10**-4.40], sigma=pylab.log10(ydata_errs), maxfev=10**5)
                fit_double, cov_double = optimize.curve_fit(dschechter, xdata, ydata, p0=[10.83, -0.5, -1.55, 10**-2.81, 10**-4.40], sigma=ydata_errs, maxfev=10**5)


                ###  fitting single
                fit_single, cov_single = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts_sf.sum(axis=0) / master_volumes_sf.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=pylab.log10((elo+ehi)[inds]/2.), maxfev=10**6)
                fit_single, cov_single = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts_sf.sum(axis=0) / master_volumes_sf.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=((elo+ehi)[inds]/2.), maxfev=10**6)
                ...
                inds_chi2 = pylab.find(-pylab.isnan(lydata) & -pylab.isinf(lydata))
                n_points_sf.append(len(inds_chi2))
                ...
                ###  adding best-fit params to array
                if vi_counter in [1, 2]:
                        ymod = dschechter(xmod, *fit_double)
                        aa = sp1.plot(xmod, ymod / dm, color=color_i, lw=1)[0]
                        fits_sf.append(fit_double)
                        covs_sf.append(cov_double.diagonal()**0.5)

                else:
                        ymod = schechter_mf(xmod, *fit_single)
                        a = sp1.plot(xmod, ymod / dm, color=color_i, lw=1)
                        fits_sf.append(fit_single)
                        covs_sf.append(cov_single.diagonal()**0.5)
        ...
        ###  writing data_table to ascii file
        outer = open('../data_after_VoronoiMC_bugfix/massFunctions_SF_Tomczak+2017.dat', 'w')
        ...
        #for vi in [0]:
                ...
                ###  fitting single
                fit_single, cov_single = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts_qu.sum(axis=0) / master_volumes_qu.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=pylab.log10((elo+ehi)[inds]/2.), maxfev=10**6)
                fit_single, cov_single = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts_qu.sum(axis=0) / master_volumes_qu.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=((elo+ehi)[inds]/2.), maxfev=10**6)
                ...
                inds_chi2 = pylab.find(-pylab.isnan(lydata) & -pylab.isinf(lydata))
                n_points_qu.append(len(inds_chi2))
                ...
                if vi_counter in [0, 1]:
                        ymod = dschechter(xmod, *fit_double)
                        aa = sp3.plot(xmod, ymod / dm, color=color_i, lw=1)[0]
                        fits_qu.append(fit_double)
                        covs_qu.append(cov_double.diagonal()**0.5)

                else:
                        ymod = schechter_mf(xmod, *fit_single)
                        a = sp3.plot(xmod, ymod / dm, color=color_i, lw=1)
                        fits_qu.append(fit_single)
                        covs_qu.append(cov_single.diagonal()**0.5)
        ...
        n_points_sf = pylab.array(n_points_sf)
        n_points_qu = pylab.array(n_points_qu)
        chi2_sf_single = pylab.array(chi2_sf_single)
        chi2_sf_double = pylab.array(chi2_sf_double)
        chi2_qu_single = pylab.array(chi2_qu_single)
        chi2_qu_double = pylab.array(chi2_qu_double)



        ###  print TeX table of Schechter params
        if True:
                print '\n'

                fits_sf_holder = fits_sf * 1
                covs_sf_holder = covs_sf * 1
                fits_qu_holder = fits_qu * 1
                covs_qu_holder = covs_qu * 1


                ###  checking if each fit is single- or double-Schechter
                for vi_counter, vi in enumerate(o_range):
                    if len(fits_sf_holder[vi_counter]) == 3:
                        fits_sf_holder[vi_counter][2] *= 10**3
                        covs_sf_holder[vi_counter][2] *= 10**3
                        order = pylab.array([1, 0, 2])

                    elif len(fits_sf_holder[vi_counter]) == 5:
                        fits_sf_holder[vi_counter][3] *= 10**3
                        fits_sf_holder[vi_counter][4] *= 10**3
                        covs_sf_holder[vi_counter][3] *= 10**3
                        covs_sf_holder[vi_counter][4] *= 10**3
                        order = pylab.array([0, 1, 3, 2, 4])

                    s = '%s & ' % voronoi_labels[vi]
                    for f, c in zip(fits_sf_holder[vi_counter][order], covs_sf_holder[vi_counter][order]):
                        s += '$%5.2f\pm%4.2f$ & ' % (f, c)
                    print s.replace('<', '$<$')[:-2] + ' \\\\'
                ...
                ###  checking if each fit is single- or double-Schechter
                for vi_counter, vi in enumerate(o_range):
                    if len(fits_qu_holder[vi_counter]) == 3:
                        fits_qu_holder[vi_counter][2] *= 10**3
                        covs_qu_holder[vi_counter][2] *= 10**3
                        order = pylab.array([1, 0, 2])

                    elif len(fits_qu_holder[vi_counter]) == 5:
                        fits_qu_holder[vi_counter][3] *= 10**3
                        fits_qu_holder[vi_counter][4] *= 10**3
                        covs_qu_holder[vi_counter][3] *= 10**3
                        covs_qu_holder[vi_counter][4] *= 10**3
                        order = pylab.array([0, 1, 3, 2, 4])

                    s = '%s & ' % voronoi_labels[vi]
                    for f, c in zip(fits_qu_holder[vi_counter][order], covs_qu_holder[vi_counter][order]):
                        s += '$%5.2f\pm%4.2f$ & ' % (f, c)
                    print s.replace('<', '$<$')[:-2] + ' \\\\'
        ...
        ###  writing data_table to ascii file
        outer = open('../data_after_VoronoiMC_bugfix/massFunctions_QU_Tomczak+2017.dat', 'w')
...
##################################################
###  Calculating for single/double Schechter fits
###   1) Reduced Chi**2
###   2) Beyesian Information Criterion
###   3) F-test
##################################################
...
if True:
    ...
    ###  storing best-fit parameters for single/double Schecther fits
    fits_single_tot, covs_single_tot = [], []
    fits_double_tot, covs_double_tot = [], []

    ###  storing chi**2 values for single/double Schecther fits
    n_points_tot = []
    chi2_single_tot = []
    chi2_double_tot = []

    ###  storing pyplot.plot() objects for plotting
    line_plot_objects_tot = []
    ...
    for vi_counter, vi in enumerate(o_range):
        ...
        ###  adding arbitrary line plot object
        xmod = numpy.linspace(x[inds].min()-dm/2., x[inds].max()+dm/2., 100)
        xmod += offie[vi_counter]
        line_plot_objects_tot.append(sp1.plot(xmod, xmod/xmod, color=color_i, lw=1)[0])
        ...
        fits_double_tot.append(fit_double)
        covs_double_tot.append(cov_double.diagonal()**0.5)


        ###  fitting single
        fit_single, cov_single = optimize.curve_fit(schechter_mf, lmassbars[inds], (master_number_counts.sum(axis=0) / master_volumes.sum(axis=0))[vi][inds], p0=[-1, 11, 10**-3], sigma=((elo+ehi)[inds]/2.), maxfev=10**6)

        fits_single_tot.append(fit_single)
        covs_single_tot.append(cov_single.diagonal()**0.5)
        ...
        inds_chi2 = pylab.find(-pylab.isnan(lydata) & -pylab.isinf(lydata))
        n_points_tot.append(len(inds_chi2))

        chi2 = ((lymod[inds_chi2] - lydata[inds_chi2])**2 / lyerr[inds_chi2]**2).sum()
        chi2_double_tot.append(chi2)




    fits_single_tot = pylab.array(fits_single_tot)
    covs_single_tot = pylab.array(covs_single_tot)

    fits_double_tot = pylab.array(fits_double_tot)
    covs_double_tot = pylab.array(covs_double_tot)

    n_points_tot = pylab.array(n_points_tot)
    chi2_single_tot = pylab.array(chi2_single_tot)
    chi2_double_tot = pylab.array(chi2_double_tot)
    ...
    ###  saving plot with single Schechter fits
    for i_densbin in range(len(fits_single_tot)):
        line_plot_obj = line_plot_objects_tot[i_densbin]
        xmod = line_plot_obj.get_xdata()
        ymod = schechter_mf(xmod, *fits_single_tot[i_densbin]) / dm
        line_plot_obj.set_ydata(ymod)

    fig.savefig('../figures/massFunctions_singleSch_tot.png')



    ###  saving plot with double Schechter fits
    for i_densbin in range(len(fits_double_tot)):
        line_plot_obj = line_plot_objects_tot[i_densbin]
        xmod = line_plot_obj.get_xdata()
        ymod = dschechter(xmod, *fits_double_tot[i_densbin]) / dm
        line_plot_obj.set_ydata(ymod)

    fig.savefig('../figures/massFunctions_doubleSch_tot.png')




    ###  calculating: Reduced Chi**2, BIC, F-test
    bics_single_tot, bics_double_tot = [], []
    for i_densbin, vi in enumerate(o_range):

        dof_single = n_points_tot[i_densbin] - 3
        dof_double = n_points_tot[i_densbin] - 5

        likelihood_single = numpy.exp(-chi2_single_tot[i_densbin] / 2)
        likelihood_double = numpy.exp(-chi2_double_tot[i_densbin] / 2)


        chi2red_single = chi2_single_tot[i_densbin] / dof_single
        chi2red_double = chi2_double_tot[i_densbin] / dof_double

        bic_single = -2 * numpy.log(likelihood_single) + 3 * numpy.log(n_points_tot[i_densbin])
        bic_double = -2 * numpy.log(likelihood_double) + 5 * numpy.log(n_points_tot[i_densbin])

        numer = (chi2_single_tot[i_densbin] - chi2_double_tot[i_densbin]) / (dof_single - dof_double)
        denom = chi2_double_tot[i_densbin] / dof_double
        fratio = numer / denom


        ###  print TeX table of single Schechter params
        s = '    %s & ' % voronoi_labels[vi].replace('<', '$<$')

        s += '$%5.2f\pm%4.2f$ & ' % (fits_single_tot[i_densbin][1], covs_single_tot[i_densbin][1])
        s += '$%5.2f\pm%4.2f$ & ' % (fits_single_tot[i_densbin][0], covs_single_tot[i_densbin][0])
        s += '$%5.2f\pm%4.2f$ & ' % (fits_single_tot[i_densbin][2]*10**3, covs_single_tot[i_densbin][2]*10**3)

        s += '... & ... & '

        s += '%4.2f & ' % chi2red_single
        s += '%4.1f & ' % bic_single

        print s[:-2] + ' \\\\'


        ###  print TeX table of double Schechter params
        s = '    %s & ' % voronoi_labels[vi].replace('<', '$<$')

        s += '$%5.2f\pm%4.2f$ & ' % (fits_double_tot[i_densbin][0], covs_double_tot[i_densbin][0])
        s += '$%5.2f\pm%4.2f$ & ' % (fits_double_tot[i_densbin][1], covs_double_tot[i_densbin][1])
        s += '$%5.2f\pm%4.2f$ & ' % (fits_double_tot[i_densbin][3]*10**3, covs_double_tot[i_densbin][3]*10**3)
        s += '$%5.2f\pm%4.2f$ & ' % (fits_double_tot[i_densbin][2], covs_double_tot[i_densbin][2])
        s += '$%5.2f\pm%4.2f$ & ' % (fits_double_tot[i_densbin][4]*10**3, covs_double_tot[i_densbin][4]*10**3)

        s += '%4.2f & ' % chi2red_double
        s += '%4.1f & ' % bic_double
        ...
        ###  changing lines in plot to fit with lower BIC
        bics_single_tot.append(bic_single)
        bics_double_tot.append(bic_double)
        if bic_single < bic_double:
            line_plot_obj = line_plot_objects_tot[i_densbin]
            xmod = line_plot_obj.get_xdata()
            ymod = schechter_mf(xmod, *fits_single_tot[i_densbin]) / dm
            line_plot_obj.set_ydata(ymod)
...
if True:
    ...
    phis_tot = [smfs_orelse_tot.phi_n05v00, smfs_orelse_tot.phi_00v05, smfs_orelse_tot.phi_05v10, smfs_orelse_tot.phi_10v15, smfs_orelse_tot.phi_15v20]
    phis_qu = [smfs_orelse_qu.phi_n05v00, smfs_orelse_qu.phi_00v05, smfs_orelse_qu.phi_05v10, smfs_orelse_qu.phi_10v15, smfs_orelse_qu.phi_15v20]
    phis_sf = [smfs_orelse_sf.phi_n05v00, smfs_orelse_sf.phi_00v05, smfs_orelse_sf.phi_05v10, smfs_orelse_sf.phi_10v15, smfs_orelse_sf.phi_15v20]

    elos_tot = [smfs_orelse_tot.elo_n05v00, smfs_orelse_tot.elo_00v05, smfs_orelse_tot.elo_05v10, smfs_orelse_tot.elo_10v15, smfs_orelse_tot.elo_15v20]
    elos_qu = [smfs_orelse_qu.elo_n05v00, smfs_orelse_qu.elo_00v05, smfs_orelse_qu.elo_05v10, smfs_orelse_qu.elo_10v15, smfs_orelse_qu.elo_15v20]
    elos_sf = [smfs_orelse_sf.elo_n05v00, smfs_orelse_sf.elo_00v05, smfs_orelse_sf.elo_05v10, smfs_orelse_sf.elo_10v15, smfs_orelse_sf.elo_15v20]

    ehis_tot = [smfs_orelse_tot.ehi_n05v00, smfs_orelse_tot.ehi_00v05, smfs_orelse_tot.ehi_05v10, smfs_orelse_tot.ehi_10v15, smfs_orelse_tot.ehi_15v20]
    ehis_qu = [smfs_orelse_qu.ehi_n05v00, smfs_orelse_qu.ehi_00v05, smfs_orelse_qu.ehi_05v10, smfs_orelse_qu.ehi_10v15, smfs_orelse_qu.ehi_15v20]
    ehis_sf = [smfs_orelse_sf.ehi_n05v00, smfs_orelse_sf.ehi_00v05, smfs_orelse_sf.ehi_05v10, smfs_orelse_sf.ehi_10v15, smfs_orelse_sf.ehi_15v20]


    offie = numpy.linspace(-0.02, 0.02, len(phis_tot))
    ...
    sp1.set_ylim(-0.05, 1.05)


    for i_densbin in range(len(phis_tot)):

        inds = numpy.where((phis_qu[i_densbin] > 0) & (phis_sf[i_densbin] > 0))

        n_qu = phis_qu[i_densbin][inds]
        n_sf = phis_sf[i_densbin][inds]
        fq = n_qu / (n_qu + n_sf)

        e_qu = (ehis_qu[i_densbin][inds] + elos_qu[i_densbin][inds]) / 2.
        e_sf = (ehis_sf[i_densbin][inds] + elos_sf[i_densbin][inds]) / 2.
        e_fq = numpy.sqrt(e_qu**2 * (n_sf / (n_qu + n_sf)**2)**2 + e_sf**2 * (n_qu / (n_qu + n_sf)**2)**2)

        c = pyplot.cm.cool(i_densbin / (len(phis_tot)-1.))

        sp1.errorbar(smfs_orelse_tot.lmass[inds] + offie[i_densbin], fq, xerr=0.25/2, yerr=e_fq,
                     ls='-', marker='o', ms=11, mfc='none', mec=c, color=c, mew=2, elinewidth=2, ecolor=c)
...
########################################
###  Plot ORELSE low-z and high-z SMFs
########################################


mfs_loz = mypy.readcat('../data_after_VoronoiMC_bugfix/massFunctions_TOT_Tomczak+2017_lowz.dat')
mfs_hiz = mypy.readcat('../data_after_VoronoiMC_bugfix/massFunctions_TOT_Tomczak+2017_highz.dat')

pdf = PdfPages('../output_after_VoronoiMC_bugfix/massFunctions_low-high-z.pdf')


phis_loz = [mfs_loz.phi_n05v00, mfs_loz.phi_00v05, mfs_loz.phi_05v10, mfs_loz.phi_10v15, mfs_loz.phi_15v20]
elos_loz = [mfs_loz.elo_n05v00, mfs_loz.elo_00v05, mfs_loz.elo_05v10, mfs_loz.elo_10v15, mfs_loz.elo_15v20]
ehis_loz = [mfs_loz.ehi_n05v00, mfs_loz.ehi_00v05, mfs_loz.ehi_05v10, mfs_loz.ehi_10v15, mfs_loz.ehi_15v20]

phis_hiz = [mfs_hiz.phi_n05v00, mfs_hiz.phi_00v05, mfs_hiz.phi_05v10, mfs_hiz.phi_10v15, mfs_hiz.phi_15v20]
elos_hiz = [mfs_hiz.elo_n05v00, mfs_hiz.elo_00v05, mfs_hiz.elo_05v10, mfs_hiz.elo_10v15, mfs_hiz.elo_15v20]
ehis_hiz = [mfs_hiz.ehi_n05v00, mfs_hiz.ehi_00v05, mfs_hiz.ehi_05v10, mfs_hiz.ehi_10v15, mfs_hiz.ehi_15v20]
...
fig = pylab.figure(figsize=(9.875, 8.375))
sp1 = fig.add_subplot(111)
...
if True:
    ...
    for i in range(len(phis_loz)):

        color_i = cmap(i/(len(phis_loz)-1.))
        ...
        inds = pylab.find(phi > 0)

        sp1.errorbar(lmass[inds]-0.01, phi[inds], xerr=dm/2., yerr=[elo[inds], ehi[inds]],
                     ls='', marker='o', mfc='b', mec='b', ecolor='b', ms=9, elinewidth=1.5,
                     label='0.55 < z < 0.90')
        ...
        sp1.errorbar(lmass[inds]+0.01, phi[inds], xerr=dm/2., yerr=[elo[inds], ehi[inds]],
                     ls='', marker='o', mfc='none', mec='r', ecolor='r', ms=9, elinewidth=1.5,
                     label='0.90 < z < 1.30')


        leg = sp1.legend(loc=3, title='%s' % voronoi_labels[i], numpoints=1)
...
if True:
    ...
    for vi_counter, vi in enumerate(o_range):
        ...
        #pylab.errorbar(x[inds], y[inds], xerr=dm/2., yerr=[elo[inds], ehi[inds]], ls='', mew=1.5, marker='o', ms=9, mfc='none', mec=color_i, elinewidth=1.5, ecolor=color_i)
        ...
        pbar = progressbar.ProgressBar(widgets=widgets, maxval=nsims+1).start()
...
##################################################
###  Plot results from merger simulation v3.0  ###
##################################################


if True:

    smfdata_orelse = mypy.readcat('../data/massFunctions_TOT_Tomczak+2017.dat')
    ...
    ##############################################################
    ###  Combining mock catalogs from each fmerge realization  ###
    ##############################################################

    if True:
        mocks = []
        directories = glob.glob('../data/simulations/fiducial/sim*')
        #directories = glob.glob('../data/simulations/fiducial_constMergerRate/sim*')
        #directories = glob.glob('../data/simulations/fiducial_fquiescentField/sim*')
        #directories = glob.glob('../data/simulations/fiducial_quenchingCountdown/sim*')
        directories.sort()

        for i_dir in range(len(directories)):

            fmerge = int(directories[i_dir].split('fmerge')[1][0:2])  # fraction of galaxies merged betwene z_start and z_final
 
            ###  reading mock catalogs
            mock_catalogs = [pickle.load(open(filename, 'rb')) for filename in glob.glob('%s/simulation*pickle' % directories[i_dir])]
            n_mocks = len(mock_catalogs)

            ###  grabbing the first to begin the combination of all mocks
            mock_catalog_combined = mock_catalogs[0]
            mock_catalog_combined.fmerge = fmerge
            mock_catalog_combined.SMFs = numpy.array(mock_catalog_combined.SMFs)
            mock_catalog_combined.fquiescent = numpy.array(mock_catalog_combined.fquiescent) / n_mocks

            ###  adding mocks together
            mock_catalogs = mock_catalogs[1:]
            for i_mock in range(len(mock_catalogs)):

                mock = mock_catalogs[i_mock]

                mock_catalog_combined.ngal_initial += mock.ngal_initial
                mock_catalog_combined.ngal_timesteps += mock.ngal_timesteps

                mock_catalog_combined.SMFs += numpy.array(mock.SMFs)
                mock_catalog_combined.fquiescent += numpy.array(mock.fquiescent) / n_mocks

                mock_catalog_combined.mass_in_ICL += mock.mass_in_ICL
                mock_catalog_combined.mass_in_mergers += mock.mass_in_mergers
                mock_catalog_combined.mass_running += mock.mass_running

            mocks.append(mock_catalog_combined)
    ...
    ##########################################
    ###  Fitting mock SMFs to ORELSE SMFs  ###
    ##########################################
    ...
    if True:
        ...
        for sp in sps123:
            ...
            sp.set_ylabel('Number / cMpc$^{3}$ / dex$^{}$')
            sp.axis([8.4, 11.9, 5*10**-4, 4*10**-1])
            sp.axis([8.4, 11.9, 5.5*10**-4, 4.5*10**-1])

        for sp in sps456:
            sp.axhline(0, color='k', ls='--')

        fig.subplots_adjust(hspace=0, wspace=0, left=0.09, bottom=0.1, top=0.96)

        voronoi_labels = ['-0.5 < log(1+$\delta_{\mathrm{gal}}$) < 0.0', 
        ...
        phis_orelse = [smfdata_orelse.phi_n05v00, smfdata_orelse.phi_00v05, smfdata_orelse.phi_05v10, smfdata_orelse.phi_10v15, smfdata_orelse.phi_15v20]
        elos_orelse = [smfdata_orelse.elo_n05v00, smfdata_orelse.elo_00v05, smfdata_orelse.elo_05v10, smfdata_orelse.elo_10v15, smfdata_orelse.elo_15v20]
        ehis_orelse = [smfdata_orelse.ehi_n05v00, smfdata_orelse.ehi_00v05, smfdata_orelse.ehi_05v10, smfdata_orelse.ehi_10v15, smfdata_orelse.ehi_15v20]


        ###  Looping over three highest overdensity SMFs
        inds_smfs = [0, 1, 2]
        inds_smfs = [2, 3, 4]
        for i, i_phi in enumerate(inds_smfs):


            lmass_orelse = smfdata_orelse.lmass[phis_orelse[i_phi] > 0]
            dm = abs(lmass_orelse[1] - lmass_orelse[0])
            phi_orelse = phis_orelse[i_phi][phis_orelse[i_phi] > 0]
            elo_orelse = elos_orelse[i_phi][phis_orelse[i_phi] > 0]
            ehi_orelse = ehis_orelse[i_phi][phis_orelse[i_phi] > 0]


            chi2s    = numpy.zeros(len(mocks))
            f_ICL    = numpy.zeros(len(mocks))
            fmergeds = numpy.zeros(len(mocks))
            scales   = numpy.zeros(len(mocks))
            bestfit_models = numpy.zeros((len(mocks), len(lmass_orelse)))

            for i_mock in range(len(mocks)):

                lmass_mock = mocks[i_mock].lmassbars
                smf_mock   = mocks[i_mock].SMFs[-1]
                smf_mock_interp = numpy.interp(lmass_orelse, lmass_mock, smf_mock)
                smf_mock_initial_interp = numpy.interp(lmass_orelse, lmass_mock, mocks[i_mock].SMFs[0])


                ###  calculating weights for SMF data
                log_errors = ((elo_orelse/phi_orelse) + (ehi_orelse/phi_orelse)) / 2. / numpy.log(10)
                weights = 1. / log_errors**2


                ###  calculating factor to scale mock SMF to ORELSE data
                numer = pylab.sum(phi_orelse * weights)
                denom = pylab.sum(smf_mock_interp * weights)

                scale = numer / denom
                scales[i_mock] = scale
                bestfit_models[i_mock] = smf_mock_interp * scale

                #pyplot.plot(lmass_orelse, smf_mock_interp * scale)


                f_ICL[i_mock] = mocks[i_mock].get_f_ICL()[-1]
                fmergeds[i_mock]= mocks[i_mock].fmerge
                chi2s[i_mock] = numpy.sum((numpy.log10(phi_orelse) - numpy.log10(smf_mock_interp * scale))**2 / log_errors**2)


            #Pchi = 1. / chi2s**2
            Pchi = numpy.exp(-0.5 * chi2s)
            Pchi_normalized = Pchi / numpy.trapz(Pchi, fmergeds)
            #pyplot.plot(fmergeds, Pchi_normalized)


            bestfit_marginalized = numpy.average(bestfit_models, axis=0, weights=Pchi)
            f_ICL_marginalized   = numpy.average(f_ICL, axis=0, weights=Pchi)
            fmerge_marginalized  = numpy.average(fmergeds, axis=0, weights=Pchi)
            scale_marginalized   = numpy.average(scales, axis=0, weights=Pchi)



            ###  plotting
            c = pyplot.cm.cool(i_phi / (len(phis_orelse)-1.))

            sps123[i].errorbar(lmass_orelse, phi_orelse, xerr=dm/2., yerr=[elo_orelse, ehi_orelse], 
                                   ls='', mew=1.5, marker='o', ms=9, mfc='none', mec=c, elinewidth=1.5, ecolor=c, label=voronoi_labels[i], zorder=2)

            sps123[i].plot(lmass_orelse, bestfit_marginalized, color='k', lw=5, zorder=1)
            sps123[i].plot(lmass_orelse, bestfit_marginalized, color='gray', lw=2, zorder=1)


            ###  residuals
            sps456[i].errorbar(lmass_orelse, pylab.log10(phi_orelse) - pylab.log10(bestfit_marginalized), 
                                   xerr=dm/2., yerr=log_errors,
                                   ls='', mew=1.5, marker='o', ms=9, mfc='none', mec=c, elinewidth=1.5, ecolor=c, label=voronoi_labels[i], zorder=2)


            ###  legend text
            sps123[i].set_title(voronoi_labels[i], y=1.015, fontsize=19)

            chi2_best = numpy.sum((numpy.log10(phi_orelse) - numpy.log10(bestfit_marginalized))**2 / log_errors**2)
            chi2red_best = chi2_best / len(lmass_orelse)
            legend_text = 'Model ($\chi^2_{\mathrm{red.}}$ = %.2f)' % chi2red_best
            legend_text += '\nf$_{merged}$ = %i%%' % fmerge_marginalized
            legend_text += '\n$M_{*,\mathrm{ICL}} \, / \, M_{*,\mathrm{total}}$ = %2i%%' % (round(f_ICL_marginalized*100))
            sps123[i].text(0.17, 0.06, legend_text, transform=sps123[i].transAxes, fontsize=16)

            y0 = 0.20
            sps123[i].plot([0.07, 0.14], [y0, y0], lw=5, color='k', transform=sps123[i].transAxes, zorder=1)
            sps123[i].plot([0.07, 0.14], [y0, y0], lw=2, color='gray', transform=sps123[i].transAxes, zorder=1)




            ###  plotting fmerged=0% state of model
            if i == 1:
                xt = lmass_orelse[0] + 0.01
                yt = bestfit_models[0][0] * 1.3
                print yt
                t = sps123[i].text(xt, yt, 'f$_{merged}$=0%', color='gray', rotation=-23, fontsize=17)
            sps123[i].plot(lmass_orelse, bestfit_models[0], lw=1., ls='--', dashes=[6, 3, 2, 3], color='gray', zorder=1)
    ...
    ######################################################
    ###  Plotting example SMFs from merger simulation  ###
    ######################################################
    ...
    if True:
        ...
        ###  figure for paper
        cmap = pyplot.cm.jet
        #fig_paper = pyplot.figure(figsize=(8.9, 7.5))
        fig_paper = pyplot.figure(figsize=(8.9, 9.1))
        sp_paper = fig_paper.add_subplot(111)
        sp_cbar = add_inset(sp_paper, [0.6, 0.85, 0.33, 0.05])
        #sp_cbar2 = add_inset(sp_paper, [0.6, 0.65, 0.33, 0.05])

        fig_paper.subplots_adjust(left=0.11, bottom=0.1)
        ...
        sp_cbar.set_title('$M_{*,\,\mathrm{ICL}} \, / \, M_{*,\,\mathrm{total}}$', fontsize=18, position=[.5, 1.25])
        sp_cbar.set_yticks([])
        imdatx, imdaty = numpy.meshgrid(numpy.linspace(mocks[0].get_f_ICL()[-1], mocks[-1].get_f_ICL()[-1], int(mocks[-1].get_f_ICL()[-1]*100)+1), [0, 0])
        sp_cbar.imshow(imdatx, cmap=cmap)
        sp_cbar.set_aspect('auto')

        xticks = sp_cbar.get_xticks()

        xticks = numpy.array([0.01, 0.04, 0.07])
        sp_cbar.set_xticks(xticks * 100)
        sp_cbar.set_xticklabels(xticks.astype(str), fontsize=15)



        ###  I want to plot 8 example SMFs that look (more or less) linearly
        ###  interpoloated between the fmerge=0% and 95% simulation runs.

        lsmf00 = numpy.log10(mocks[0].SMFs[-1] + 1)
        lsmf95 = numpy.log10(mocks[-1].SMFs[-1] + 1)

        y_datapoints_all = numpy.array([])

        n = 8
        for i_iter in range(n):


            weights = [1 - i_iter/(n-1.), i_iter / (n-1.)]
            lsmf_interp = numpy.average([lsmf00, lsmf95], axis=0, weights=weights)



            ###  plotting
            inds = mocks[0].lmassbars > 8.5
            c = cmap(i_iter / (n-1.))
            sp_paper.plot(mocks[0].lmassbars[inds], 10**lsmf_interp[inds], marker='o', ms=4, mew=0.5, color=c, zorder=2)


            ###  Fitting mock SMFs to interped SMF to determine what the representative fmerge is
            chi2s    = numpy.zeros(len(mocks))
            fmergeds = numpy.zeros(len(mocks))
            ...
            for i_mock in range(len(mocks)):
                ...
                ###  calculating factor to scale mock SMF to ORELSE data
                numer = pylab.sum(10**lsmf_interp)
                denom = pylab.sum(smf_mock)
                scale = numer / denom

                fmergeds[i_mock]= mocks[i_mock].fmerge
                chi2s[i_mock] = numpy.sum((lsmf_interp - numpy.log10(smf_mock * scale))**2)


            #Pchi = numpy.exp(-0.5 * chi2s)
            Pchi = numpy.exp(-0.5 * chi2s / chi2s.min())

            fmerge_marginalized  = numpy.average(fmergeds, axis=0, weights=Pchi)

            font = FontProperties()
            font.set_family('sans-serif')
            t = sp_paper.text(8.65-0.05, 10**lsmf_interp[inds][0], '%.1f%%' % fmerge_marginalized,
                              fontproperties=font, 
                              horizontalalignment='right', verticalalignment='center')

            y_datapoints_all = numpy.append(y_datapoints_all, 10**lsmf_interp[inds])


        ###  setting axis limits of plot
        sp_paper.set_xlim(8.05, 11.75)
        sp_paper.set_ylim(y_datapoints_all.min()/20, y_datapoints_all.max()*10)


        ###  plotting z_start SMF
        sp_paper.plot(mocks[0].lmassbars[inds], mocks[0].SMFs[0][inds], color='gray', lw=1.5, ls='-', dashes=[6,3,2,3], zorder=1, label='initial state\nz$_{start}$ = 5')
        t = sp_paper.text(11, 600, 'initial state\nz$_{start}$ = 5', fontsize=18, color='gray',
        ...
        font = FontProperties()
        font.set_family('sans-serif')
        y0 = mocks[0].SMFs[-1][lmass_mock == 8.75][0]
        sp_paper.text(8.65-0.2, y0*2.3, 'fraction\nmerged', fontproperties=font, 
                      verticalalignment='bottom', horizontalalignment='center', fontsize=22)
...
if False:
    ...
    outname = '../output_after_VoronoiMC_bugfix/MF_sfqui.pdf'
    pdf = PdfPages(outname)
    ...
    outname2 = '../output_after_VoronoiMC_bugfix/spec_completeness.pdf'
...
#############################################
###  Plots for Keck Science Meeting Talk  ###
#############################################
...
if True:
    ...
    if True:
        ...
        for i, i_phi in enumerate(inds_smfs):
            ...
            legend_text += '\n$M_{*,\mathrm{ICL}} \, / \, M_{*,\mathrm{total}}$ = %2i%%' % (round(f_ICL_marginalized*100))
            #sps123[i].text(0.17, 0.06, legend_text, transform=sps123[i].transAxes, fontsize=16)
            sps123[i].text(0.07, 0.06, legend_text, transform=sps123[i].transAxes, fontsize=16)
